...
5. `es_evento` (22/09): the prompt also marks `false` for a post that thanks/recaps something already past ("gracias a quienes vinieron...") with no future date invited, even if it's clearly about bioconstrucción — otherwise these sat in `pendiente_confirmacion` forever with no date to filter on. **Fixed (22/09):** a post that thanks/recaps the past *and* announces a concrete upcoming event in the same text was being marked `false` too — the Jev-vs-Gemini comparison caught this as a real classification bug (see ROADMAP.md), not just a comparison artifact. The prompt now marks `true` for the mixed case and extracts the future event's data, ignoring the thank-you part.
