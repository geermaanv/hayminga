...
// ---- Aviso de evento publicado al organizador (vía email) ----

// Llamada por src/hiker_pipeline.py cuando encuentra un email público en
// el perfil de Instagram que originó el evento — el evento ya se publicó
// directo (confianza suficiente: cuenta con email de contacto público),
// esto es solo el aviso + los dos CTA que importan (ver ROADMAP.md,
// 09/2026). No hay nada que confirmar/rechazar por click — si algo está
// mal, el organizador responde el mail y se corrige a mano; probamos así
// primero antes de construir un link de edición. Mismo secreto que
// "subir_imagen": es solo para el pipeline, no para el público.
function avisarEventoPublicado_(data) {
  var secreto = PropertiesService.getScriptProperties().getProperty('SUBIR_IMAGEN_SECRETO');
  if (!secreto || data.secreto !== secreto) {
